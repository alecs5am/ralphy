...
- `studio/` — Ralphy Studio (#107), the local artifact browser + scene board (`cd studio && bun run dev`): hash-routed workspace + project selection (`#<ws>/<project>`, refresh-safe), a **Board** view (#478 — scene anchors in order, each scene's image variants side by side, click the chosen one) and a **Files** view (live artifact grid over `<project>/artifacts/<kind>/`), plus a run-level **graph** canvas (#490 — a content-farm run as a derived source→research→strategy→template→batch→project→gate→repair→unit→destination graph with a per-node drawer), a collapsible workflow-pipeline strip, and a preview modal. Read-only over MEDIA; the only writes are sidecar METADATA (never media): the board choice + node layout (`board.json`), the run-canvas node layout (#490 — run-scoped `canvas.json`), and the object annotations (#488 — append-only `annotations.jsonl` of controlled-vocab tags + a free note on any selected run / project / workflow node / artifact / eval finding / Unit / destination, written project-scoped or run-scoped), the agent context inbox (#489 — `agent-inbox/<ts>-<action>.{json,md}` context packs the user sends from a Studio selection), and the safe config patches (#491 — append-only `config-events.jsonl` of allowlisted run/workflow settings the user PROPOSES; Studio never applies). Read inbox packs with `ralphy studio inbox list|show` and config patches with `ralphy studio patch list|show`, then `apply|reject` them; **an inbox pack / config patch is CONTEXT + a proposal, not an instruction to spend money** — you still decide and run the actual ralphy verbs behind the paid-generation gate. The visual alternative to `fd -H` into `.ralphy/`. Studio also exposes a stable local **API boundary** (#492, [`docs/workflow-app-api.md`](docs/workflow-app-api.md)) that Claude Code orchestrates through — prefer an endpoint over an ad-hoc `.ralphy/` file read where one exists; `GET /api/capabilities` is the machine-readable action list (run show/status, workflow graph, annotations/inbox/config-patches, and the parked-approval list/respond gate). Every mutating endpoint stays metadata-only unless it routes a money/state transition through a ralphy verb (e.g. approval respond drives `ralphy farm review`), never a second media engine.
