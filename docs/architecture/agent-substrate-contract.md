...
1. **Product / site → ad-creative pack.** Brief names a brand URL → site-grounding sub-agent crawls it (AGENTS.md #15, [`../playbooks/site-grounding.md`](../../.agents/skills/researcher/references/site-grounding.md)) → `classifyContentMode` returns `ad-creative-pack`/`fb-creatives` → the [`fb-creatives`](../../.agents/skills/fb-creatives/SKILL.md) skill scaffolds the 5-set matrix → parallel `ralphy generate image` with `--ref` on every gen → `ralphy unit create` packages the numbered PNGs. Proves: research → mode → format-match → generate → Unit.
