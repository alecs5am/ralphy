...
Add required presentation `position` with unique `(unit_revision_id, position)` alongside platform uniqueness, and `presentation_items.created_at`. Persist presentation array order exactly. Fix the Unit author Session trigger to resolve scope through `units.workspace_id` with nullable Project rather than inner-joining Projects, so Workspace Units accept only active Workspace Sessions while Project Units accept a Workspace Session or exact-Project Session. Add persistent primary/logical-key insert guards for revisions and every child so `INSERT OR REPLACE` cannot bypass sealed immutability with `recursive_triggers=OFF`.

`reviseUnit` requires the exact current head (null first); later omitted parent defaults current while an explicit older sealed same-Unit parent allows branching. It inserts a complete revision graph in one transaction: at least one ordered heterogeneous item, ordered unique-platform presentations, optional cover/crop/safe-area/options, and optional ordered item-override subsets addressed by base item position. Cover must be an Artifact revision already present among base items. Override input/output positions are unique/contiguous and missing base positions reject; keep base and override configs separately rather than silently merging. It then seals the revision, conditionally updates the Unit head/row version, and appends activity. Caption/item/presentation edits create another Unit revision; do not expose post-creation setters that mutate an old graph. Validate exact-one target, contiguous unique positions, scope, parent, Session, and Project Iteration. Platform is a non-empty kebab slug; reject new `reels`/`shorts` IDs because migration maps them to `instagram`/`youtube`.

Publications are append-only attempts with required social account/Run, provider identifiers, state, URL, validated schedule/publish timestamps, error, and a required globally unique idempotency key. Target is the Presentation platform; do not duplicate it in another column. Require the account's canonical platform to match the Presentation. A duplicate key for the same presentation/account/provider returns the original attempt with its original schedule and audits an idempotent skip; the Run may differ on legitimate replay. Reusing a key for another target/account/provider conflicts. An existing draft means an interrupted provider call to resume, never proof of a successful post. Validate exact Unit/account/Run ownership: Workspace Unit requires Workspace-only Run, Project Unit exact-Project Run. Allowed operational states are `draft`, `scheduled`, `submitted`, `published`, `failed`, and `cancelled`; updates require the expected state and the locked forward transition graph. Postiz immediate acceptance is `submitted`, scheduled acceptance is `scheduled`; retry after failure creates a new attempt/key.

Metric snapshots are immutable time-series rows with indexed common metrics and validated raw provider JSON. Add duplicate-ID/REPLACE, no-update, and no-delete triggers; use SQLite integer-type checks for captured time and common non-negative counters. Retain provider-specific fields in raw JSON.

Lock Publication identity/provenance fields and the forward transition graph in SQLite as well as store code; forbid deletion and primary/idempotency-key REPLACE. This is operational state on one append-only attempt, not a mutable identity row.

- [ ] **Step 3: Cover platform and analytics behavior**

Test an eight-image carousel, a 32-sticker pack, an article, a Workspace Unit, presentation-order round trip, a shared video with one Object/Artifact/item and three TikTok/Instagram/YouTube presentations, override ordering/config separation, cover membership, scope/Session rejection, optimistic head/selection conflicts, raw-SQL update/delete/same-ID/same-logical-key REPLACE immutability, Postiz submitted/scheduled/idempotent/crash-resume/partial-target attempt history, failed then separate successful publication, and two metric snapshots whose first row cannot update/delete/replace. Reject fractional/negative/non-finite normalized counters while preserving provider values in raw JSON.
