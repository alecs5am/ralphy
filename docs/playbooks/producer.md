...
5. **Template-suggest first, but never auto-apply.** For every new project request, run `ralphy template suggest "<utterance>" --limit 3` and **surface** the top results to the user as a question with the landing-page link (https://www.alecs5am.com/templates#<slug>). Wait for user confirmation before any `ralphy template use` invocation. Below-threshold matches → just announce "drafting freeform" and proceed. Full discipline in intake playbook's "Cold-start template suggestion" section.
