...
5. **Niche-skill first; templates are remix-only.** For a new project request, match a niche skill (`/ralphy-ugc-*`) to the brief — never auto-suggest a template. A template enters only on an explicit remix pointer (`@template:<slug>`, "remix this", named slug), via `ralphy template use <slug>`. Don't run `ralphy template suggest` on cold start. Full discipline in the intake playbook's "Cold-start niche-skill match" section + [`docs/skills-vs-templates.md`](../skills-vs-templates.md). (Batch is the exception — it fans N variations off ONE base the user already chose; see batch.md.)
