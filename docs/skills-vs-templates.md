...
Templates live in the **public content library** (a static `library.json` — committed at `landing/lib/library-v2/library.json`, served on Bunny CDN and by `/library`, read by the CLI via `cli/lib/library/client.ts`) and in `.ralphy/workspaces/<ws>/templates/<slug>/` (user-local, the active workspace tier). The repo-public `templates/<category>/<slug>/` folder was retired in #084. Slugs resolve via `ralphy template list / show / suggest / use` across both tiers; filter by format with `ralphy template list --format <f>` and `ralphy template suggest "<brief>" --format <f>`. Two `kind`s ship: `vibe-reference` (full production) and `vibe-style` (prompt cookbook).
...
- **Operational workflows** — `researcher`, `evaluator`, `install`, `postmortem`, `templater`. Each has a deterministic input → output contract and a backing `ralphy` verb.
  - **`templater` = extract + classify (+ blueprint + de-dup), not push.** It reads a finished project's `units/*/unit.json` (the Unit source of truth, formed by `ralphy unit`, issue #069) and decomposes the project into the five content entities — Unit + the four typed blocks Template / Style / Recipe / Asset (the #063 model, shapes in [`landing/lib/library-v2/types.ts`](../landing/lib/library-v2/types.ts)). It applies the **recipe-vs-tag split (#082/#083)**: a candidate is a Recipe block ONLY if it carries an extractable artifact (ffmpeg filtergraph / HyperFrames snippet / bake-or-encode recipe / prompt technique) authored into `recipeKind`+`body`+`artifact`+`params`+`demo`; otherwise it is a Tag — a `tags[]` descriptor on the Unit, never a block (never publish an empty `refs:0` recipe). It matches each candidate against existing library blocks first and proposes NEW blocks only for genuine gaps. Its output is the classified entity bundle plus a per-unit Blueprint and the ordered publish runbook (optionally a local `.ralphy/workspaces/<ws>/templates/<slug>/` artifact); it does **not** push to the library. Full rule: [`.agents/skills/templater/references/recipe-vs-tag.md`](../.agents/skills/templater/references/recipe-vs-tag.md).
  - **The #056 publish skill / `landing/scripts/publish-entity.ts` = the library writer.** It takes templater's classified entities and pushes them: `--unit <dir>` publishes a finished Unit (its media → Bunny CDN, the unit appended/replaced by id in the committed `library.json`), `--block` / `--block-file` publishes a standalone Style / Recipe / Asset on its own, `--blueprint <dir>` a reproduction blueprint. Each upload also pushes the updated `library.json` to Bunny so the CLI sees it. No Supabase. Both modes are first-class and independent. The maintainer one-shot that runs the publish is `dev-publish-template`.
  - **`ralphy unit` = how a project forms Units.** It copies curated `artifacts/` picks into `<project>/units/<slug>/` + `unit.json` with provenance attached (#069). This is the project-side mirror of the library Unit; templater reads it, publish pushes it. Unit formation is **phase 17** of the canonical [Unit lifecycle](playbooks/unit-lifecycle.md) — only form/publish a Unit whose render passed the native-video final gate (`ralphy project status <id> --contract` → `polished === true`, #411).
- **Maintainer / dev tooling** — `dev-issues`, `dev-loop`, `dev-publish-template`, `dev-release`, `dev-tasks`, `normalize-skills` (`namespace: maintainer`).
...
Skills live under `.agents/skills/<slug>/` (Claude Code slash commands). Slugs carry **no `ralphy-` prefix**; audience is marked by the `namespace` frontmatter field (`user` default, `maintainer` for maintainer-only workflows).
...
| Lives in | public library (`library.json` on Bunny CDN) + `.ralphy/workspaces/<ws>/templates/<slug>/` | `.agents/skills/<slug>/` |
...
- [`cli/lib/schemas/unit.ts`](../cli/lib/schemas/unit.ts) + `ralphy unit` — how a project forms Units (#069); `landing/scripts/publish-entity.ts` — the library writer (#056): appends to the committed `library.json` + uploads media and the JSON to Bunny CDN. templater extracts/classifies into these entities; the publish primitive pushes them.
