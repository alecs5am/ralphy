...
Templates live in the **public content library** (a static `library.json` — committed at `landing/lib/library-v2/library.json`, served on Bunny CDN and by `/library`, read by the CLI via `cli/lib/library/client.ts`) and in `workspace/templates/<slug>/` (user-local). The repo-public `templates/<category>/<slug>/` folder was retired in #084. Slugs resolve via `ralphy template list / show / suggest / use` across both tiers; filter by format with `ralphy template list --format <f>` and `ralphy template suggest "<brief>" --format <f>`. Two `kind`s ship: `vibe-reference` (full production) and `vibe-style` (prompt cookbook).
...
| Lives in | public library (`library.json` on Bunny CDN) + `workspace/templates/<slug>/` | `.agents/skills/<slug>/` |
