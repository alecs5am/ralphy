...
export function CanvasCatalog({ catalog, loading, modality, selected, opener = null, onChoose, onClose, onRefresh, onOpenProviders }: {
  ...
  onChoose(model: CanvasModelDescriptor): void; onClose(): void; onRefresh(): void; onOpenProviders?(): void;
}) {
  ...
  return <Dialog.Root open modal={false} onOpenChange={(open) => { if (!open) onClose(); }}><GenerationPickerMenu kind="model" opener={opener} query={query} onQuery={setQuery} onClose={onClose}
    ...
    footer={onOpenProviders || catalog.errors.length ? <div className="generation-picker-footer">{catalog.errors.map((error) => <span className="type-xs text-muted" key={error}>{error}</span>)}{onOpenProviders && <button className={STUDIO_BUTTON} type="button" onClick={onOpenProviders}>Manage providers</button>}</div> : undefined}>
      <div className="px-2 py-2 font-code type-mono-xs text-muted" role="status">{loading ? "Loading models…" : `${models.length} models`}</div>
      {models.map((model) => <button key={`${model.provider}:${model.modality}:${model.id}`} className="generation-model-option group" type="button" title={model.name} aria-pressed={selected?.id === model.id && selected.provider === model.provider} onClick={() => onChoose(model)}><span className="generation-model-tile"><ModelBrand modelId={model.id} /></span><span className="generation-row-copy"><strong>{model.name}</strong><small>{catalog.providers.find((item) => item.id === model.provider)?.label ?? model.provider} · {model.available ? "Connected" : "Connect to run"} · {model.modality}</small></span><Check size={12} className="shrink-0 opacity-0 group-aria-pressed:opacity-100" /></button>)}
