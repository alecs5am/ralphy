...
export interface DynamicIslandFeed { rootEpoch?: number; projectStatus: Availability<ProjectStatusSummary>; activeTask: IslandTask | null; notifications: Availability<IslandNotification[]> }
...
export function projectDynamicIslandFeed(input: DynamicIslandProjectionInput): DynamicIslandFeed {
  ...
  return {
    ...
    notifications: input.appError
      ? { status: "ready", value: [{ id: `app-error-${input.rootEpoch}-${input.appError}`, title: input.appError, timestamp: Date.now(), severity: "error", unread: true }] }
