...
- **Skills** are slash-command-invocable shims under `.agents/skills/<name>/SKILL.md`. The user can type `/ralphy-researcher` and the agent loads the skill body. They're thin pointers — most of the body is "Read `docs/playbooks/<name>.md` before acting."
...
The seven skills shipped today: `ralphy-researcher`, `ralphy-evaluator`, `ralphy-install`, `release`, `postmortem`, `ralphy-remotion`, `skill-creator`. The eleven playbooks: `intake`, `researcher`, `scenarist`, `art-director`, `editor`, `producer`, `core`, `ralphy-install`, `remotion`, plus the per-domain references.
