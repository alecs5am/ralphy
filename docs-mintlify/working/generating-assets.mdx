...
Defaults are the live values from [`cli/commands/generate.ts`](https://github.com/alecs5am/ralphy/blob/main/cli/commands/generate.ts) as of v0.1.0. Always cross-check [`MODELS.md`](https://github.com/alecs5am/ralphy/blob/main/MODELS.md) before assuming — Claude's training is stale, and Ralphy reads `MODELS.md` before every call for a reason.
...
ralphy generate voiceover \
  ...
  --text "Меня зовут Анна, и я завариваю лучший кофе в городе." \
  --stability 0.55 \
  --style 0.25
```
...
- [Prompting philosophy](/prompting/philosophy) — how to write the `--prompt` text
