...
- **`ralphy` is the only entry point.** No raw `curl`, no `bunx remotion`, no ad-hoc `ffmpeg`. Every model call, render, and project mutation flows through a `ralphy` verb so the cost rollup, the manifest, and the gen-log stay honest.
