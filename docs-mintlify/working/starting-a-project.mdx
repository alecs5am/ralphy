...
Ralphy stops here. **No paid generation happens until you say "go"** (or "поехали", or equivalent). If the plan is wrong, push back — "another approach", "не так", "drop scene 3", "switch to seedance for the reaction". Ralphy redrafts and waits again. This wait is invariant in the protocol; the appstore postmortem traced a 70-minute wasted batch directly to skipping plan-approval.
...
If you want to fire the whole batch instead, say "не спрашивай каждый раз" or "fire the whole batch". Ralphy switches to batch mode for *that project*; the preference doesn't generalize.
...
3. **`/ralph-evaluator`** skill on the final mp4 — emits `eval.json` and `eval-report.md` with scene-by-scene scoring and retention check.
