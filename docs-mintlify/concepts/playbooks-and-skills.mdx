...
| [`editor.md`](https://github.com/alecs5am/ralphy/blob/main/docs/playbooks/editor.md) | "compose the video", "render", captions, transitions, audio mix, Remotion code edits |
...
| [`remotion.md`](https://github.com/alecs5am/ralphy/blob/main/docs/playbooks/remotion.md) | Remotion API details — captions, transitions, audio, ffmpeg, animation |
...
| URL drop, "analyze @handle", "what's trending in X" | `researcher.md` (and the `/ralphy-researcher` skill) |
| "write a script", scenario feedback | `scenarist.md` |
| "generate prompts / assets", model swap | `art-director.md` |
| "compose the video", "render", "captions" | `editor.md` (then `remotion.md` for API specifics) |
| End-to-end, batch (N ≥ 3), "save as template" | `producer.md` |
| "evaluate / score / QA" a rendered mp4 | `/ralphy-evaluator` skill |
...
|---|---|---|
| [`ralphy-evaluator`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-evaluator/SKILL.md) | `ralphy` | rendered mp4 → `eval.json` + `eval-report.md` |
| [`ralphy-researcher`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-researcher/SKILL.md) | `ralphy` | URLs / handles / topic → `report.md` + `sources.json` |
| [`ralphy-templater`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-templater/SKILL.md) | `ralphy` | shipped project → vibe-reference template under `templates/<category>/<slug>/` |
| [`ralphy-install`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-install/SKILL.md) | `ralphy` | fresh machine → installed binary, configured keys, working `ralphy doctor` |
| [`ralphy-remotion`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-remotion/SKILL.md) | `ralphy` | Remotion API question → topic-specific sub-doc |
| [`ralphy-postmortem`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-postmortem/SKILL.md) | `ralphy` | iteration session → 6-file postmortem set under `workspace/projects/<id>/postmortem/` |
| [`ralphy-dev-release`](https://github.com/alecs5am/ralphy/blob/main/.agents/skills/ralphy-dev-release/SKILL.md) | `ralphy-dev` | main-branch HEAD → new GitHub Release + brew + npm publication |
...
|---|---|---|
| `ralphy` | end users | `ralphy-evaluator`, `ralphy-researcher`, `ralphy-templater`, `ralphy-install` |
| `ralphy-dev` | maintainers / contributors | `release`, `ralphy-remotion`, `skill-creator` |

The install wizard (`ralphy skill install`) installs the `ralphy` set by default. `ralphy skill install --dev` opts into the maintainer set. The split exists so a tester running `/<TAB>` on a fresh install does not see `/release` (which ships the binary, not the user's project) or `/skill-creator` (a maintainer authoring tool).

A skill in `ralphy-dev` ships through the same lint and installer plumbing as `ralphy` — it is the same format, just gated behind the `--dev` flag in the installer.

## Playbooks vs. skills

The two layers solve different problems.

**Playbooks** are *role docs*. They are read by the agent when a user intent matches a row in the routing table. They cover roles (`scenarist`, `art-director`, `editor`, `producer`, `core`) and domains (`remotion`, `intake`). No frontmatter, no activation contract — they are just Markdown the agent reads.

**Skills** are *workflows*. They have a deterministic input → output contract and a single CLI command. They are slash-invocable (`/ralphy-evaluator <path.mp4>`) and they produce specific artifacts on disk. The agent invokes a skill when the user types the slash command, or when the routing table points the agent at a skill (e.g. "evaluate this video" → `/ralphy-evaluator`).
...
2. Pick a namespace — `ralphy` for user-facing, `ralphy-dev` for maintainer-only.
