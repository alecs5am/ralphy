...
A valid `.agents/skills/ralph-evaluator/SKILL.md`:
...
EN: "evaluate this video", "is this ready to ship".
RU: "оцени рендер", "что не так с видео".
```
...
```
.agents/skills/
  ralph-evaluator/SKILL.md
  ralph-researcher/SKILL.md
  ralphy-install/SKILL.md
  release/SKILL.md
...
| `namespace` | no | `ralphy` or `ralphy-dev` | drives install wizard visibility |
...
`ralphy skill install` installs only `ralphy` skills by default. `--dev` opts into the maintainer set. The agents-md lint (`scripts/lint-agents-md.ts`) reads `namespace:` and skips the "is this skill referenced from the routing table" check for `ralphy-dev` skills — maintainer skills are explicit-invocation-only.
...
The description is the **user-facing summary** of the skill, not an auto-route trigger phrase list. Claude Code renders it in the `/ralphy:<skill>` slash-command menu, the "suggest this skill" surface, and the `ralphy skill list` output. A scannable paragraph beats a wall of synonyms.
...
For longer skills (the existing `ralph-evaluator` and `ralph-researcher`), the description ends with `See body for ALSO FIRE / DO NOT FIRE / HARD INVARIANTS.` and those clauses move into the body under `## Trigger refinements` and `## Hard invariants`. This pattern keeps the description scannable while preserving the routing-fidelity detail the agent needs once the skill fires.
...
- `namespace`, if set, is `ralphy` or `ralphy-dev`.
...
- [Cookbook: new skill](/cookbook/new-skill) — full walkthrough.
