...
export const CONTENT_MODES: Record<ContentMode, ContentModeEntry> = {
  ...
  "personal-clipper": {
    ...
    implementationUnit: {
      ...
      skills: ["evaluator"],
      guidelines: [],
      cliVerbs: ["ref pull", "ref transcribe", "clip", "generate captions", "render"],
      note: "editor-stage clip route: `ralphy ref transcribe` for the word-level transcript → the agent picks highlight windows → `ralphy clip --from --to [--vertical]` cuts each window → `ralphy generate captions` + `ralphy render`. The mode playbook carries the source/duration limits + the no-good-clips-found stop.",
    },
    supportedFormats: ["video"],
