...
test("same-tick starts submit once and stale polls cannot overwrite start or cancellation", async () => {
  ...
  const staleStartPoll = deferred<CanvasRun[]>(), staleCancelPoll = deferred<CanvasRun[]>();
  ...
  vi.spyOn(bridge, "loadCanvasRuns").mockReturnValueOnce(staleStartPoll.promise).mockReturnValueOnce(staleCancelPoll.promise).mockResolvedValue([run("cancelled")]);
  ...
  try {
    ...
    await act(async () => staleStartPoll.resolve([]));
    ...
    await act(async () => staleCancelPoll.resolve([run("running")]));
...
test("completed runs stop polling and focus can refresh expired media tokens", async () => {
  ...
  const load = vi.spyOn(bridge, "loadCanvasRuns").mockResolvedValueOnce([withMedia("running", "ralphy-media://asset/first")]).mockResolvedValueOnce([withMedia("succeeded", "ralphy-media://asset/first")]).mockResolvedValue([withMedia("succeeded", "ralphy-media://asset/refreshed")]);
  const host = await harness();
  try {
    await host.render();
    await act(async () => vi.advanceTimersByTimeAsync(1800));
