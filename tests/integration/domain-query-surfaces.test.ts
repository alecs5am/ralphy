...
import { saveUnitMedia } from "../../cli/lib/store/unit-media.js";
...
describe("activity payload safety", () => {
  ...
  test("stores no forbidden raw payload across every exercised domain writer", async () => {
    ...
    const workspace = createWorkspace({ slug: "generated-safety", name: "Generated safety" });
    const directory = path.join(fixture.rootDir, ".ralphy", "media-library", "canvases", workspace.id, "assets");
    fs.mkdirSync(directory, { recursive: true });
    const filePath = path.join(directory, "output.png"); fs.writeFileSync(filePath, "generated bytes");
    const referencePath = path.join(directory, "reference.png"); fs.writeFileSync(referencePath, "reference bytes");
    await saveUnitMedia({ context: { workspaceId: workspace.id }, key: "e".repeat(64), kind: "image", name: "Generated safety", file: { path: fs.realpathSync(filePath), name: "output.png", mime: "image/png" }, references: [{ path: fs.realpathSync(referencePath), name: "reference.png", mime: "image/png" }], provenance: { prompt: "private-generation-prompt-marker", modelId: "test/image", parameters: { aspectRatio: "16:9" } } });
    ...
    const stored = db
      ...
      .all();
    expect(JSON.stringify(stored)).not.toContain("private-generation-prompt-marker");
