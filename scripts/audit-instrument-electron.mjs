...
async function runOneHiddenInstance() {
  ...
  try {
    ...
    await evaluate(cdp, `(() => { const trigger=document.querySelector('button[aria-label="Notch activity"]'); trigger?.focus(); trigger?.click(); return Boolean(trigger); })()`);
    await delay(100);
    const islandOpen = await evaluate(cdp, `Boolean(document.querySelector('[data-instrument-overlay="dynamic-island"]'))`);
    const island = await evaluate(cdp, `(() => { const trigger=document.querySelector('button[aria-label="Notch activity"]'); return { trigger:Boolean(trigger), expanded:trigger?.getAttribute('aria-expanded'), mock:Boolean(document.querySelector('.dynamic-island[data-mock]')), testFeed:Boolean(document.querySelector('.dynamic-island-test-label')) }; })()`);
    ...
    await delay(100);
    const keyboard = await evaluate(cdp, `(() => { const trigger=document.querySelector('button[aria-label="Notch activity"]'); return { focusReturned:document.activeElement===trigger, overlayClosed:!document.querySelector('[data-instrument-overlay="dynamic-island"]') }; })()`);
