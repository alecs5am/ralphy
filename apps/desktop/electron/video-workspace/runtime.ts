...
import { blankVideoComposition, videoHtml, type VideoWorkspaceAsset, type VideoWorkspaceLoad, type VideoWorkspaceRef, type VideoWorkspaceRender } from "../../shared/video-workspace";
...
export async function copyVideoFile(source: string, destination: string): Promise<void> {
  ...
  try {
    ...
    if (!info.isFile() || info.size > FILE_LIMIT) throw new Error("Choose a regular media file under 1 GB");
    ...
    try {
      ...
      for (;;) {
        ...
        if (!bytesRead) break;
        if ((total += bytesRead) > FILE_LIMIT) throw new Error("Media file exceeds 1 GB");
        let offset = 0;
...
async function assetDirectory(directory: string): Promise<string> {
  ...
  if (!info.isDirectory() || info.isSymbolicLink()) throw new Error("Invalid asset directory");
  return path;
}
async function assetsFor(runtime: VideoRuntime, directory: string, value: VideoStored): Promise<VideoWorkspaceAsset[]> {
  ...
  const sources = [{ directory, prefix: "_ralphy_assets" }, ...(value.draft.checkoutPath ? [{ directory: await safeVideoPath(runtime.root, value.draft.checkoutPath), prefix: "" }] : [])];
...
async function hydrate(runtime: VideoRuntime, ref: VideoWorkspaceRef, value: VideoStored, revision: string | null): Promise<VideoWorkspaceLoad> {
  ...
  const result: VideoWorkspaceLoad = { ...value, draftPath: join(await videoDirectory(runtime.root, ref), "draft.json"), draft: { ...value.draft, assets }, revision };
...
export async function loadVideo(runtime: VideoRuntime, ref: VideoWorkspaceRef): Promise<VideoWorkspaceLoad> {
  ...
  if (unit.projectId !== ref.projectId) throw new Error("Unit belongs to another project");
  ...
  if (unit.compositionId) {
    ...
    if (latest && !/^[a-zA-Z0-9_-]+$/.test(latest.id)) throw new Error("Invalid composition revision");
    ...
    if (!checkout.id || !checkout.checkoutPath) throw new Error("Installed runtime did not return an editable checkout");
  ...
  if (sourceWarning && sourceKind === "empty") throw new Error("The Unit’s editable source is unavailable and it has no original video. Restore its composition source before opening the editor.");
...
export async function saveVideo(runtime: VideoRuntime, ref: VideoWorkspaceRef, html: string, fps: number, expected: string | null): Promise<VideoWorkspaceLoad> {
  ...
  if (!saved) throw new Error("Open the video before saving");
  const { value } = saved;
  const changed = value.draft.html !== html;
  const versions = changed ? [{ id: videoHash(value.draft.html), html: value.draft.html, savedAt: value.draft.updatedAt }, ...value.versions].slice(0, 8) : value.versions;
  const next = { ...value, draft: { ...value.draft, html: videoHtml(html), fps, updatedAt: Date.now() }, versions };
...
export async function importVideoAsset(runtime: VideoRuntime, ref: VideoWorkspaceRef, path: string): Promise<VideoWorkspaceAsset> {
  if (!isAbsolute(path) || path.includes("\0") || path.length > 4096 || !videoMime(path)) throw new Error("Choose an image, video, audio or font file");
...
export async function renderVideo(runtime: VideoRuntime, ref: VideoWorkspaceRef, expected: string): Promise<VideoWorkspaceRender> {
  const directory = await videoDirectory(runtime.root, ref);
  if (renders.has(directory)) throw new Error("A render is already running for this video");
  renders.add(directory);
  try {
    const saved = await readVideoStore(directory);
    if (!saved || saved.revision !== expected) throw new Error("Save your current draft before rendering");
    const value = saved.value, draft = value.draft, context = contextFor(ref);
    if (!draft.compositionId || !draft.compositionRevisionId || !draft.checkoutPath) throw new Error("This Unit has no linked Composition. Ask the agent to link a video Composition before rendering.");
    const current = await runtime.request("composition.show", { context, compositionId: draft.compositionId });
    if (current.latestRevisionId !== draft.compositionRevisionId) throw new Error("The agent created a newer composition. Reopen its version before rendering these edits.");
    ...
    if (draft.checkoutHash && videoHash(original) !== draft.checkoutHash) throw new Error("The composition source changed outside this editor. Your saved draft is preserved; reopen the source before rendering.");
