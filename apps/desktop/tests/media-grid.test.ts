...
describe("media grid geometry and scheduling", () => {
  test("groups compact audio controls for vertical centering without changing the full viewer", async () => {
    const view = await mounted(createElement(AudioWaveform, { src: "ralphy-media://preview/audio", name: "Voiceover", compact: true }));
    try {
      expect(view.host.container.findAll((node) => (node.getAttribute("class") ?? "").split(" ").includes("audio-compact-content"))).toHaveLength(1);
      await view.rerender(createElement(AudioWaveform, { src: "ralphy-media://preview/audio", name: "Voiceover" }));
      expect(view.host.container.findAll((node) => (node.getAttribute("class") ?? "").split(" ").includes("audio-compact-content"))).toHaveLength(0);
    } finally { await view.unmount(); }
  });

  test("reports an unplayable source once, from the only player there is", async () => {
    const onError = vi.fn();
    const view = await mounted(createElement(AudioWaveform, { src: "ralphy-media://asset/audio-token", name: "Sonic hook", onError }));
    try {
      const element = bySrc(view.host.container, "ralphy-media://asset/audio-token")[0]!;
...
describe("mounted media tiles", () => {
  ...
  test("hands the audio slot on only when the mounted player reports its metadata", async () => {
    const bytes = MAX_WAVEFORM_DECODE_BYTES + 1;
