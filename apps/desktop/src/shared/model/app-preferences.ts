...
export function writeAppPreferences(storage: PreferenceStorage, values: AppPreferences): void {
  ...
  queueMicrotask(() => { if (typeof window !== "undefined") window.dispatchEvent(new Event("ralphy:preferences-changed")); });
}
...
export function useAppPreferences(storage: PreferenceStorage): AppPreferencesController {
  ...
  useEffect(() => {
    const refresh = () => setValues(readAppPreferences(storage));
    window.addEventListener("storage", refresh);
    window.addEventListener("ralphy:preferences-changed", refresh);
    return () => {
      window.removeEventListener("storage", refresh);
      window.removeEventListener("ralphy:preferences-changed", refresh);
    };
  }, [storage]);
