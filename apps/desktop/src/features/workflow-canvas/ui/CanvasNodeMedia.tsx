...
export function CanvasNodeResults({ data }: { data: CanvasNodeData }) {
  ...
  return <div className="flex min-w-0 flex-col gap-2">
    ...
    {selected.kind === "text" && <div className="flex items-center justify-between gap-2"><span className="canvas-node-meta">{selected.text?.length.toLocaleString() ?? 0} characters{selected.asset ? " in preview" : ""}</span><button className={NODE_ACTION} type="button" onClick={() => { void navigator.clipboard.writeText(selected.text ?? "").then(() => setCopyState("Copied"), () => setCopyState("Could not copy text")); }} aria-label={selected.asset ? "Copy text preview" : "Copy result text"}><Copy size={12} />{selected.asset ? "Copy preview" : "Copy"}</button></div>}
