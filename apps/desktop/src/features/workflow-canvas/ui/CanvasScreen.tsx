...
export function CanvasScreen({ onOpenUnit, workspaceId, workspaceName, storageScope, agentBusy, onRequestAgent, onOpenProviders, onOpenAgents, headerHost: explicitHeaderHost, windowAction }: {
  ...
  const runtime = useCanvasRuntime(workspaceId, draft?.id, draft?.nodes.flatMap((node) => node.kind === "variation" && node.config?.selectedResultId ? [node.config.selectedResultId] : []) ?? []);
  ...
  const addNode = (kind: CanvasNodeKind, model?: CanvasModelDescriptor) => {
    ...
    if (model) { node.title = model.name; node.config = { ...node.config, modelId: model.id, provider: model.provider, operation: model.operation }; }
  ...
  const useResult = (result: CanvasRunResult) => {
    if (!draft || draft.nodes.length >= CANVAS_LIMIT) return;
    if (result.unavailableReason) { runtime.setError(result.unavailableReason); return; }
  ...
  return <InstrumentScreenRoot descriptor={canvasInstrumentStates} state={screenState}><main ref={screen} tabIndex={-1} onKeyDown={keyboard} className="canvas-screen @container/canvas relative flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden text-ink outline-none" aria-label="Working canvases">
    ...
    {!draft ? <PageHeaderHost.Provider value={headerHost ?? null}><CanvasLibrary workspaceName={workspaceName} items={editor.items} loading={editor.loading} onCreate={editor.create} onSelect={editor.select} onReload={() => { void editor.reload(); }} /></PageHeaderHost.Provider> : <>
      ...
      <div className="canvas-stage relative flex min-h-0 min-w-0 flex-1 overflow-hidden rounded-window" ref={stage}>
        ...
        {catalogNode && <CanvasCatalog key={catalogNode} selected={runtime.catalog.models.find((model) => model.id === inspectedNode?.config?.modelId && model.provider === inspectedNode?.config?.provider && model.modality === inspectedNode?.config?.modality)} opener={modelOpener.current} catalog={runtime.catalog} loading={runtime.loadingModels} modality={draft.nodes.find((node) => node.id === catalogNode)?.config?.modality} onChoose={(model) => { const node = draft.nodes.find((item) => item.id === catalogNode); if (node) patchNode(node.id, { title: model.name, config: { modality: model.modality, modelId: model.id, provider: model.provider, operation: model.operation, variants: node.config?.variants ?? 1 } }); setCatalogNode(null); }} onClose={() => setCatalogNode(null)} onRefresh={() => { void runtime.refreshModels(); }} onOpenProviders={onOpenProviders} onOpenAgents={onOpenAgents} />}
        {showRuns && <CanvasRunsPanel onOpenUnit={onOpenUnit} hasOlder={runtime.hasOlder} loadingOlder={runtime.loadingOlder} onLoadOlder={runtime.loadOlder} runs={runtime.runs} selectedRun={runtime.selectedRun} expanded={resultsExpanded} running={busy} onSelectRun={runtime.setSelectedRunId} onClose={() => setShowRuns(false)} onExpand={() => setResultsExpanded((value) => !value)} onCancel={(id) => { void runtime.cancel(id); }} onExport={(result) => { if (result.asset) void bridge.exportGenerationAsset(workspaceId, result.asset).catch((cause) => runtime.setError(cause instanceof Error ? cause.message : String(cause))); }} onUseResult={useResult} onRestore={(run) => { editor.edit({ ...run.snapshot, id: draft.id, name: draft.name }); setResultsExpanded(false); }} onExecute={() => { void start("execute"); }} />}
