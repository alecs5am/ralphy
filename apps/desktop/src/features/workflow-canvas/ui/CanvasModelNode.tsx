  ...
  return <div className="canvas-generation-controls">
    <fieldset className="generation-form-fields nodrag nopan" disabled={data.disabled}>
      <button className="generation-model-row" type="button" data-connected={model?.available} data-state={data.choosingModel ? "open" : "closed"} aria-label={`Choose generation model for ${node.title}`} disabled={!data.onChooseModel} onClick={(event) => data.onChooseModel?.(node.id, event.currentTarget)}>
        <span className="generation-model-tile"><ModelBrand modelId={settings.modelId} /></span><span className="generation-row-copy"><strong>{model?.name || settings.modelId || "Choose a model"}</strong><small>{model ? `${model.provider} · ${model.available ? "Connected" : "Connection needed"} · ${model.modality}` : "Browse your creative engines"}</small></span><ChevronDown size={12} />
      </button>
    ...
    <div className="generation-footer">
      ...
      {!model?.available && <span className="type-xs text-muted">{model ? "Unavailable · check provider settings" : settings.modelId ? "Not checked" : "Not configured"}</span>}
