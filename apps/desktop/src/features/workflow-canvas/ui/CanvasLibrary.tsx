...
function StarterArtwork({ template }: { template: CanvasTemplate }) {
  ...
  return <svg className="h-12 w-full text-canvas-library-accent" viewBox="0 0 320 160" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.5">
...
export function CanvasGraphPreview({ canvas }: { canvas: WorkflowCanvas }) {
  if (!canvas.nodes.length) return <div className="flex h-20 items-center justify-center text-muted"><Plus size={28} strokeWidth={1.2} aria-hidden="true" /></div>;
  ...
  return <svg className="h-20 w-full text-muted" viewBox={`${minX - 90} ${minY - 90} ${width + 180} ${height + 180}`} role="img" aria-label={`${canvas.name}: ${canvas.nodes.length} ${canvas.nodes.length === 1 ? "node" : "nodes"} and ${canvas.edges.length} ${canvas.edges.length === 1 ? "connection" : "connections"}`}>
...
export function CanvasLibrary({ workspaceName, items, onCreate, onSelect, onReload, loading = false }: {
  ...
  return <main className="canvas-library @container/canvas-library flex min-w-0 flex-1 flex-col gap-2 overflow-y-auto p-2 text-ink" aria-label="Canvas library">
    ...
    <section aria-labelledby="canvas-starters-title"><div className="mb-2 flex items-center justify-between gap-2"><h2 className="m-0 type-sm font-medium" id="canvas-starters-title">New canvas</h2><span className="type-xs text-muted">Templates</span></div><div className="canvas-starters-row flex min-w-0 gap-2 overflow-x-auto p-0.5">{STARTERS.map(({ id, icon: Icon, title, steps }) => <button className={`${WINDOW} canvas-starter w-48 flex-none text-left text-ink hover:bg-chip focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ink @min-canvas-wide/canvas-library:w-auto @min-canvas-wide/canvas-library:flex-1`} data-template={id} type="button" key={id} disabled={loading} onClick={() => onCreate(id)}><span className={`${WINDOW_PLATE} w-full bg-canvas-library-tint`}><StarterArtwork template={id} /></span><span className="flex flex-1 flex-col gap-1 p-2"><span className="flex items-center gap-2"><Icon size={15} className="shrink-0 text-canvas-library-accent" /><strong className="type-sm font-medium">{title}</strong></span><small className="mt-auto pt-1 font-code type-mono-xs text-muted">{steps}</small></span></button>)}</div></section>
    <section aria-labelledby="canvas-saved-title"><div className="mb-2 flex items-center justify-between gap-2"><h2 className="m-0 type-sm font-medium" id="canvas-saved-title">Your canvases <span className="ml-2 type-xs font-normal text-muted">{items.length}</span></h2><span className="font-code type-mono-xs text-muted">Saved in this workspace</span></div>
      {loading ? <div className="grid grid-cols-2 gap-2" role="status" aria-label="Loading canvases">{[0, 1].map((index) => <div className="h-36 animate-pulse rounded-inner bg-field motion-reduce:animate-none" key={index} />)}</div>
        : items.length ? <div className="grid grid-cols-1 gap-2 @min-canvas-wide/canvas-library:grid-cols-2 @min-canvas-library-wide/canvas-library:grid-cols-3">{items.map((item) => <button className={`${WINDOW} text-left hover:bg-chip focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink`} type="button" key={item.canvas.id} onClick={() => onSelect(item)}><span className={`${WINDOW_PLATE} w-full p-2`}><CanvasGraphPreview canvas={item.canvas} /></span><span className={`${WINDOW_TITLEBAR} w-full`}><Workflow size={16} className="shrink-0 text-muted" /><span className="flex min-w-0 flex-1 flex-col gap-1"><strong className="truncate type-sm font-medium text-ink">{item.canvas.name}</strong><small className="type-xs text-muted">{item.canvas.nodes.length} nodes · {item.canvas.edges.length} connections</small></span><ArrowUpRight size={14} className="shrink-0 text-muted" /></span></button>)}</div>
          : <div className="flex min-h-20 items-center gap-3 rounded-inner border border-dashed border-divider p-3"><span className="grid size-10 shrink-0 place-items-center rounded-inner bg-field text-muted"><Workflow size={22} strokeWidth={1.3} /></span><span className="flex flex-col gap-2"><strong className="type-sm font-normal">Your next project starts above.</strong><p className="m-0 type-xs leading-relaxed text-muted">Choose a starting point. Your saved canvases will live here.</p></span></div>}
    </section>
  </main>;
}
