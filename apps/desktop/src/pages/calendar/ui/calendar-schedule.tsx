...
export function ScheduleDialog({ open, unit, units, accounts, step, initialDate, timezone, postizAvailable, saving, onOpenChange, onSelect, onStep, onSave, onReconnect, onOpenUnit }: { open: boolean; unit: CalendarReadyUnitDto | null; units: CalendarReadyUnitDto[]; accounts: CalendarWorkspaceDto["accounts"]; step: "content" | "settings"; initialDate: string | null; timezone: string; postizAvailable: boolean; saving: boolean; onOpenChange(open: boolean): void; onSelect(unit: CalendarReadyUnitDto): void; onStep(step: "content" | "settings"): void; onSave(submit: boolean, at: number, channels: CalendarChannelInput[], unitRevisionId: string): void; onReconnect(accountId: string | null): void; onOpenUnit(): void }) {
  ...
  const payload = (): CalendarChannelInput[] => selectedChannels.map((channel) => ({ presentationId: channel.presentationId, socialAccountId: channel.socialAccountId, settings: settings[channelKey(channel)] ?? {} }));
  ...
  return <Modal
    ...
    actions={<button type="button" className={`calendar-open-unit ${ACTION} h-7 px-2.75 type-sm bg-surface text-ink hover:bg-surface-hover disabled:cursor-not-allowed disabled:opacity-35 ${OVERLAY_RING}`} disabled={!unit} onClick={onOpenUnit}><ArrowUpRight className={ICON} />Open Unit</button>}
  >
    <div className="calendar-modal-layout grid min-h-0 flex-1 grid-cols-(--calendar-modal-columns) gap-5.5 overflow-y-auto px-6 pb-5 pt-4">
      ...
      {step === "content" ? <section className="calendar-schedule-form flex min-w-0 flex-col gap-4">
        ...
        <section className="calendar-channel-section flex flex-col gap-1.75"><header className="flex items-center gap-2.25"><span className={MODAL_FIELD_LABEL}>CHANNELS</span><small className="font-code type-mono-md text-muted">{selectedChannels.length} of {channels.filter((channel) => !disconnectedIds.has(channel.socialAccountId)).length} available selected{channels.some((channel) => disconnectedIds.has(channel.socialAccountId)) ? ` · ${channels.filter((channel) => disconnectedIds.has(channel.socialAccountId)).length} needs reconnect` : ""}</small><i className="flex-1" /><button type="button" className={`${ACTION} h-6 px-2.25 type-label bg-surface-sunken text-ink hover:bg-surface disabled:cursor-not-allowed disabled:opacity-35 ${OVERLAY_RING}`} disabled={!unit} onClick={() => onStep("settings")}><SlidersHorizontal className={ICON_MD} />Platform settings</button></header><div className="calendar-modal-channels grid grid-cols-2 gap-1.5">{channels.map((channel) => { const id = channelKey(channel); const Icon = platformIcon(channel.platform); const disconnected = disconnectedIds.has(channel.socialAccountId); const chosen = selectedIds.includes(id); const content = <><AccountMark identity={`${channel.platform}:${channel.account}`} /><span className={MODAL_ROW_COPY}><strong className={`flex items-center gap-1.5 truncate type-ui font-normal ${chosen ? "text-on-instrument" : "text-ink"}`}><Icon className={`${ICON_LG} flex-none ${chosen ? "text-on-instrument-muted" : "text-muted"}`} />{channel.account}</strong><small className={`truncate font-code type-mono-sm ${chosen ? "text-on-instrument-muted" : "text-muted"}`}>{disconnected ? "Reconnect in Postiz to publish" : `${capitalize(channel.platform)} · available`}</small></span>{disconnected ? <button type="button" className={`${ACTION} h-5.5 px-2.25 type-xs bg-surface text-ink hover:bg-surface-hover ${OVERLAY_RING}`} onClick={() => onReconnect(channel.socialAccountId)}>Reconnect</button> : <i className={`calendar-check-box ${CHECK_BOX} ${chosen ? "bg-on-instrument" : "bg-surface"}`}>{chosen && <Check className={`${ICON_MD} ${CHECK_MARK_ON_INSTRUMENT}`} strokeWidth={2.6} />}</i>}</>; return disconnected ? <div className={`calendar-channel-option is-disconnected ${MODAL_ROW} bg-surface-sunken text-ink`} key={id}>{content}</div> : <button type="button" className={`calendar-channel-option ${MODAL_ROW} ${chosen ? " is-selected bg-instrument text-on-instrument" : " bg-surface-sunken text-ink hover:bg-surface"}`} key={id} onClick={() => toggleChannel(id)}>{content}</button>; })}</div></section>
        <p className="m-0 type-sm leading-prose text-muted">Each channel uses the caption saved on the selected Unit revision. Review or edit it in the Unit before scheduling.</p>
        {channels.length === 0 && <p className="m-0 type-sm leading-prose text-muted">No connected accounts match this Unit’s presentations. Connect Postiz, then check the Unit’s publishing platforms.</p>}
...
export function ReconnectDialog({ open, account, credential, saving, error, onCredential, onOpenChange, onSave }: { open: boolean; error: string | null; account: CalendarWorkspaceDto["accounts"][number] | null; credential: string; saving: boolean; onCredential(value: string): void; onOpenChange(open: boolean): void; onSave(): void }) {
  return <Modal
    ...
    title={account ? `Reconnect ${account.handle}` : "Connect Postiz"}
    titleClassName="m-0 min-w-0 flex-1 truncate type-heading font-normal text-ink"
    description="Verify a Postiz API key and connect publishing accounts to this workspace."
    descriptionClassName="sr-only"
    closeLabel="Close reconnect"
    bodyClassName="calendar-reconnect-card gap-4 p-5"
  ><>
    <p className="m-0 type-sm leading-row text-muted">{account ? "Reconnect the channel in Postiz, then verify a key that can access this account." : "Connect channels in Postiz first, then paste its API key. Ralphy imports enabled Instagram, YouTube, TikTok, X, and Telegram accounts into this workspace."}</p>
    <label className={`flex flex-col gap-1.75 ${OVERLAY_FIELD_RING}`}><span className={MODAL_FIELD_LABEL}>POSTIZ API KEY</span><input className={`${MODAL_INPUT} h-9`} type="password" autoComplete="off" maxLength={4096} aria-label="Postiz API key" value={credential} placeholder="Paste the scoped Postiz key" onChange={(event) => onCredential(event.target.value)} /></label>
    <small className="font-code type-mono-sm leading-prose text-muted">The key is checked with Postiz and stored encrypted for this workspace. This step reads account details and does not publish content.</small>
    {error && <p role="alert" className="m-0 type-sm text-state-danger">{error}</p>}
    <footer className="flex justify-end gap-2"><Dialog.Close asChild><button type="button" className={OVERLAY_ACTION}>Cancel</button></Dialog.Close><button type="button" className={`calendar-primary ${OVERLAY_ACTION_PRIMARY} disabled:cursor-not-allowed disabled:opacity-35`} disabled={saving || credential.trim().length < 8} onClick={onSave}>{saving ? "Checking…" : account ? "Save and reconnect" : "Connect accounts"}</button></footer>
  </></Modal>;
}
...
export function PlatformFields({ platform, values, onChange }: { platform: string; title: string; values: Record<string, JsonValue>; onChange(key: string, value: JsonValue): void }) {
  if (platform === "instagram") return <SettingsSegment label="PUBLISH AS" options={["post", "story"]} value={String(values.instagramPostType ?? "post")} onChange={(next) => onChange("instagramPostType", next)} />;
  if (platform === "youtube") return <SettingsSegment label="VISIBILITY" options={["public", "unlisted", "private"]} value={String(values.youtubeVisibility ?? "public")} onChange={(next) => onChange("youtubeVisibility", next)} />;
  if (platform === "tiktok" || platform === "x") return <SettingsCheck label="Made with AI" checked={values.madeWithAi === true} onChange={(next) => onChange("madeWithAi", next)} />;
  return <p className="m-0 type-sm text-muted">This platform uses the Unit caption and Postiz defaults. No extra settings are available here.</p>;
}
