/**
 * Projecting the main process' catalogs into what the Marketplace renders, and the filters over
 * the result.
 *
 * Every projection is total: a model, a public item and a pack entry all become the same
 * presentation, so the grid renders one shape and nothing downstream branches on origin except
 * where origin is the point. A field the source did not report stays absent rather than becoming
 * a plausible default.
 */
import type {
  LocalModelCatalog,
