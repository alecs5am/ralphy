...
export function MediaGallery({ items, project, rootEpoch, selectedRef, resolvePreview, onSelect, onOpen, onContextMenu, hasMore, loadingMore, appendError, onLoadMore, onRetryAppend }: {
  items: MediaCardDto[];
  ...
  return <div className="media-gallery flex min-h-0 min-w-0 flex-1 flex-col gap-1.5 overflow-hidden">
    ...
    <div className="media-gallery-rail flex min-w-0 flex-none gap-1.5 overflow-x-auto overflow-y-hidden overscroll-contain pb-1" aria-label="Gallery thumbnails" ref={attachRail}>
      {items.map((card) => <div className="media-gallery-thumb w-24 shrink-0" key={`${card.ref.type}:${card.ref.id}`}>
        <MediaCardTile card={card} project={project} rootEpoch={rootEpoch} selected={card.ref.type === selected.ref.type && card.ref.id === selected.ref.id} resolvePreview={resolvePreview} actionLabel={`Show ${mediaCardName(card)}`} onSelect={() => onSelect(card)} onOpen={() => onSelect(card)} onContextMenu={(point) => onContextMenu(card, point)} />
      </div>)}
