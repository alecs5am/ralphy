...
function Section({ title, children, open = true, className = "" }: {
  title: string;
    ...
    <Dialog.Content asChild onCloseAutoFocus={(event) => { event.preventDefault(); restoreFocus(); }}>
      ...
      >
        <header className={`shared-inspector-head ${WINDOW_TITLEBAR}`}>
          <span className="min-w-0 flex-1 truncate font-code type-mono-sm tracking-caps text-muted">{detail.kind} · {detail.mime ?? "Unknown file type"}</span>
        ...
        <div className={`shared-inspector-scroll gap-3.5 overflow-y-auto px-4 pt-3 pb-4.5 ${WINDOW_BODY}`}>
          <div>
            ...
            <Dialog.Description asChild><p className="mx-0 mt-0.75 mb-2.25 font-code type-mono-md text-muted">Asset · {detail.slug}</p></Dialog.Description>
            <div className="flex flex-wrap gap-1.25"><span className={TITLE_CHIP}>Type · {detail.kind}</span><span className={TITLE_CHIP}>Revision status · {detail.selectedState ?? "Not selected"}</span></div>
          </div>
          ...
          {detail.preview === "no-target" && <p className={`-mt-2 ${REASON}`} id={targetlessActionId}>Open original is unavailable because this item has no selected file.</p>}
          ...
          <Section title="Revisions">
            ...
            {revisions.status === "ready" && revisions.items.length === 0 && <p>No revisions yet.</p>}
            ...
            <p>Selecting a revision changes the default for future use. Existing project references keep their saved version.</p>
          ...
          <Section title="Referenced as" className="shared-inspector-referenced-as">
            {detail.referencedAs.length > 0
              ? <ul>{detail.referencedAs.map((role) => <li key={role}>{role}</li>)}</ul>
              : <p>No referenced roles recorded.</p>}
          </Section>
