/* The vocabulary the workspace-overview sections share: every section is one widget on
...
import { MODAL_ACTION } from "@/shared/ui/Modal";
...
export const SECTION = `${WINDOW} workspace-overview-section gap-1 text-ink`;
export const SECTION_HALF = `${SECTION} @min-workspace-section/instrument-desk:col-span-6`;

/* Panel titles use the shared Window titlebar, with an icon and readable identity. */
export const SECTION_TITLE = "m-0 type-sm font-semibold text-ink";
...
export const ROW_THREE = "grid grid-cols-(--workspace-attention-columns) gap-3 @max-workspace-row/main-region:grid-cols-(--workspace-glyph-columns)";
