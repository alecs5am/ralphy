...
export function ContextSidebar({
  ...
  return (
    ...
    <aside className="context-sidebar flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden rounded-window bg-panel px-0.5 pb-0.5 text-ink animate-sidebar-in motion-reduce:animate-none">
      {/* The 32px row starts at the shell's 4px inset, sharing its center with the native
          window controls. The empty run reserves their space; Electron draws the controls. */}
      ...
      <div className="sidebar-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain pt-3 pb-2">
        {workspace && <WorkspacePagesNav pages={["generation", "units", "canvas", "calendar", "shared"]} page={page} pageActive={pageActive} workspace={workspace} onOpenPage={onOpenPage} />}
      ...
      <footer className="sidebar-footer mx-3 mt-2 mb-3 flex h-7.5 flex-none items-center gap-1">
