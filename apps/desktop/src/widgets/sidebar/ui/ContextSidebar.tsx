...
export function ContextSidebar({
  ...
  const displayName = preferences["profile.displayName"].trim() || profileIdentity(rootPath ?? "");
  ...
  return (
    ...
    <aside className="context-sidebar flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden rounded-window bg-panel px-0.5 pb-0.5 text-ink animate-sidebar-in motion-reduce:animate-none">
      ...
      {(deskLens || chatLens) && <div className="sidebar-search mx-3 mb-3 flex h-10 flex-none items-center gap-2.5 rounded-full bg-field px-3.25">
        ...
        <input
          ...
          type="search"
          value={query}
          placeholder={chatLens ? "Search chats…" : "Search…"}
          aria-label={chatLens ? "Search chats" : "Search the sidebar"}
      ...
      <div className="sidebar-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain">
        ...
        {mode === "work" && workspace && chatLens && <SidebarChats key={workspace.id} chats={chats} query={needle} activeChatId={activeChatId} now={now} onSelectChat={onSelectChat} onRenameChat={onRenameChat} onArchiveChat={onArchiveChat} />}
      ...
      {rootPath && <div className="sidebar-footer flex h-15.5 flex-none items-stretch px-3 pb-3 [&_.instrument-profile-control]:h-full [&_.instrument-profile-control]:w-full">
        <InstrumentProfileControl
          identity={{ displayName, initials: displayName.slice(0, 2).toUpperCase(), avatarUrl: null }}
