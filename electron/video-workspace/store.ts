...
import { videoHtml, videoRef, type VideoWorkspaceRef, type VideoWorkspaceDraft, type VideoWorkspaceVersion, type VideoWorkspaceRender } from "../../shared/video-workspace";
...
export async function videoDirectory(root: string, rawRef: VideoWorkspaceRef): Promise<string> {
  ...
  for (const part of ["media-library", "video-workspaces", ref.workspaceId, ref.projectId, ref.unitId]) {
    ...
    if (!info.isDirectory() || info.isSymbolicLink()) throw new Error("Video workspace storage must be a regular directory");
  }
  return path;
}
export async function safeVideoPath(root: string, path: string): Promise<string> {
  ...
  if (!local || local.startsWith("..") || local.startsWith("/")) throw new Error("Video source is outside this library");
  let checked = base;
  for (const part of local.split("/")) {
    checked = join(checked, part);
    if ((await lstat(checked)).isSymbolicLink()) throw new Error("Video sources cannot contain symbolic links");
  }
  return checked;
}
export async function readVideoText(path: string, limit = VIDEO_BYTES): Promise<string> {
  ...
  try {
    ...
    if (!info.isFile() || info.size > limit) throw new Error("Video source exceeds the size limit");
...
export async function readVideoStore(directory: string): Promise<{ value: VideoStored; revision: string } | null> {
  ...
  if (value.draft?.schemaVersion !== 1 || !Array.isArray(value.versions) || !Array.isArray(value.draft.assets)) throw new Error("This video draft cannot be opened");
...
export async function writeVideoStore(directory: string, value: VideoStored, expected: string | null, assertCurrent: () => void): Promise<string> {
  ...
  await guardedAtomicWrite(join(directory, "draft.json"), raw, {
    ...
    beforeReplace: async () => {
      if ((await readVideoStore(directory))?.revision !== (expected ?? undefined)) throw new Error("This video changed outside this editor. Reload before saving; your current edits are still here.");
    },
  });
...
export async function videoFiles(directory: string, prefix = "", depth = 0): Promise<string[]> {
  ...
  for (const entry of await readdir(join(directory, prefix), { withFileTypes: true })) {
    ...
    if (files.length > 300) throw new Error("This composition contains too many source files to open");
  }
  return files;
}
